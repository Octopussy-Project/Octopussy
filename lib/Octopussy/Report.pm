    ...
    if (@report_restriction_list)
    {
        foreach my $c (@confs)
        {
            foreach my $res (@report_restriction_list)
            {
                ...
                {
                    my $cat =
                        (defined $c->{category} ? $c->{category} : 'various');
                    $category{$cat} =
...
sub Table_Creation
{
        ...
        || ($query =~ /SELECT (.+) FROM .+/i))
    {
        ...
        foreach my $f (@data)
        {
            ...
            $f =~ s/\S+ AS (Plugin_\S+__\S+)$/$1/i
                ;                     # for fields modified by plugins
            $f =~ s/ AS .+$//i;
            $f =~ s/^\s*\(?COUNT\(//gi;
            $f =~ s/^\s*\(?DISTINCT\(//gi;
            $f =~ s/^\s*\(?SUM\(//gi;
...
sub Generate
{
    ...
    if ($type eq 'array')
    {
        ...
        Octopussy::Report::HTML::Generate($outputfile, $rc->{name}, $begin,
            $end, $devices, $services, $data, $rc->{columns},
            $rc->{columns_name}, $stats, $lang);
        my $file_xml = Octopussy::FS::File_Ext($outputfile, 'xml');
        Octopussy::Report::XML::Generate($file_xml, $rc->{name}, $begin, $end,
            $devices, $services, $data, $rc->{columns}, $rc->{columns_name},
            $stats, $lang);
...
sub File_Info_Tooltip
{
    ...
    if (-f $file)
    {
        ...
        $ttip =
            AAT::Translation::Get($lang, '_DEVICES') . ": $c->{devices}<br>";
        $ttip .=

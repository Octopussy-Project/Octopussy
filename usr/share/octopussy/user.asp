	...
	else
	{
		$Session->{AAT_MSG_ERROR} =
    		AAT::User::Add("Octopussy", $login, $f->{password}, 
				...
				$f->{status});
		AAT::Syslog::Message("octo_WebUI", "GENERIC_CREATED", "User", 
			$login, $Session->{AAT_LOGIN})
  			if (NOT_NULL($Session->{AAT_MSG_ERROR}));
		if ($f->{certificate} == 1)
 		{
  			my %conf = ( country => "FR", state => "Ile de France",
			city => "Paris", organization => "Octopussy Project", 
			organizational_unit => "Devel",

...
sub Syslog_By_DeviceType_Graph
{
    ...
    if (-f $RRD_SYSLOG_DTYPE)
    {
        ...
        my $cmd  = "$NICE_RRDGRAPH "
            . Graph_Parameters("$DIR_RRD_PNG/${file}.png", "-$length", '-120',
            $title, $GRAPH_WIDTH, $GRAPH_HEIGHT, 'Logs by Device Type');
        ...
        foreach my $dt (Octopussy::Device::Types())
        {
            ...
            $dt =~ s/[-\s+]/_/g;
            $cmd .=
                "DEF:$dt=\"$RRD_SYSLOG_DTYPE\":$dt:AVERAGE CDEF:cdef$dt=$dt ";
            $cmd .=
...
{
    my $return =
        Syslog_By_DeviceType_Graph('syslog_dtype_hourly', 'Hourly Stats',
        $HOURLY);
...
{
    my $return =
        Syslog_By_DeviceType_Graph('syslog_dtype_weekly', 'Weekly Stats',
        $WEEKLY);
...
{
    my $return =
        Syslog_By_DeviceType_Graph('syslog_dtype_yearly', 'Yearly Stats',
        $YEARLY);
...
sub Syslog_By_Device_Service_Taxonomy_Graph
{
    ...
    my $cmd = "$NICE_RRDGRAPH "
        . Graph_Parameters("$DIR_RRD_PNG/${file}.png", "-$length", '-120',
        $title, $GRAPH_WIDTH, $GRAPH_HEIGHT, "Logs Taxonomy for $service");
...
sub Syslog_By_Device_Taxonomy_Graph
{
    ...
    foreach my $taxo (Octopussy::Taxonomy::List())
    {
        ...
        $cdef =~ s/,$//;
        $legend .=
              Graph_Line($t, $type, $color, $type_name) . ' '
            . Graph_Legend($t) . ' ';
        $first = 0;
    }
    $cmd .= " $def $cdef $legend";
        ...
        foreach my $l (ARRAY($data))
        {
            if ($l->{$tl} >= $start)
            {
                ...
                if (NOT_NULL($key))
                {
                    $ds{$key} = 1;
                    my $block =
                        int(($l->{$tl} - $start) / $rrd_step_mins); ## no critic

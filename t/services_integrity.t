...
my %msg = (
  ...
  MSG_PATTERN_DUPLICATED_FIELD => "Service: %s - Duplicated field %s in msgid %s",
  ...
  MSG_PATTERN_PID => "Service: %s - msg pattern of %s contains pid field not of PID type",
  MSG_RANK_BAD_FORMAT => "Service: %s - Bad rank format for msgid %s",
  MSG_RANK_DUPLICATED => "Service: %s - Duplicated msg rank %s between %s and %s",
...
sub Msg
{
  ...
  $error{$level}{$type} = (AAT::NOT_NULL($error{$level}{$type}) ?
    $error{$level}{$type} + 1 : 1);
}
...
sub Check_Service_Message_Rank
{
  ...
  {
    Msg("ERROR", "service", "MSG_RANK_DUPLICATED",
      $service, $rank, $msgid, $seen_rank->{"$i_rank"});
  }
  else
    { $seen_rank->{"$i_rank"} = $msgid; }
  if (($i_rank < 1) || ($i_rank > $nb_msg))
  {
    Msg("ERROR", "service", "MSG_RANK_OUT_OF_RANGE",
      $service, $rank, AAT::Padding($nb_msg, 3));
  }
}
...
sub Check_Service_Message_Pattern
{
  ...
  foreach my $f (@fields)
  {
    ...
    $fcount{$f->{name}} = (AAT::NOT_NULL($fcount{$f->{name}})
      ? $fcount{$f->{name}} + 1 : 1);
  }
  foreach my $fc (sort keys %fcount)
  {
    Msg("ERROR", "service", "MSG_PATTERN_DUPLICATED_FIELD",
      $service, $fc, $msgid)
      if ($fcount{$fc} > 1);
    Msg("ERROR", "service", "MSG_PATTERN_FIELD_UNKNOWN",
      $service, $fc, $msgid)
      if (AAT::NULL($tf{$fc}));
  }
...
sub Check_Service_Messages
{
  ...
  foreach my $m (@{$conf->{message}})
  {
    my ($msgid, $rank, $m_table, $m_loglevel, $m_taxo, $pattern) =
      ($m->{msg_id}, $m->{rank}, $m->{table}, $m->{loglevel},
      $m->{taxonomy}, $m->{pattern});
    ...
    Msg("ERROR", "service", "MSG_TABLE_UNKNOWN",
      $service, $msgid, $m_table)
      if (AAT::NULL($table->{$m_table}));
    Msg("ERROR", "service", "MSG_LOGLEVEL_UNKNOWN",
      $service, $msgid, $m_loglevel)
      if (AAT::NULL($loglevel->{$m_loglevel}));
    Msg("ERROR", "service", "MSG_TAXONOMY_UNKNOWN",
      $service, $msgid, $m_taxo)
...
foreach my $service (@services)
{
  if ($service !~ /^[a-z][a-z0-9_-]+$/i)
    ...
    { Check_Service_Messages($service, \%table, \%loglevel, \%taxo); }
}
print "->Checker_Services:\n";
print "  Errors: $error{ERROR}{service} Warnings: $error{WARNING}{service}\n";

ok($error{ERROR}{service} == 0 && $error{WARNING}{service} == 0, 'Services Integrity') or diag($str_error);
